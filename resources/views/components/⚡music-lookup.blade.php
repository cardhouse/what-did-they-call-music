...
<div class="min-h-screen bg-zinc-50 dark:bg-zinc-950">
    <div class="max-w-5xl mx-auto px-4 sm:px-6 py-6 space-y-8 md:space-y-12">
        <header class="text-center space-y-4 md:space-y-6 pt-6 md:pt-12 pb-4 md:pb-6">
            <div class="inline-flex items-center justify-center p-2 px-3 md:px-4 rounded-full bg-blue-500/10 text-blue-600 dark:text-blue-400 font-medium text-xs md:text-sm mb-2 md:mb-4 animate-in fade-in zoom-in duration-700">
                <flux:icon icon="musical-note" class="size-4 mr-1.5 md:mr-2" />
                <span class="hidden sm:inline">The Ultimate </span>Music Time Machine
            </div>
            <div class="space-y-2">
                <flux:heading size="xl" level="1" class="text-3xl sm:text-4xl md:text-5xl font-black tracking-tight bg-gradient-to-br from-zinc-900 to-zinc-500 dark:from-white dark:to-zinc-500 bg-clip-text text-transparent">
                    What Did They Call Music?
                </flux:heading>
                <flux:text size="lg" variant="subtle" class="max-w-lg mx-auto text-sm md:text-base px-2">
                    Discover which hits defined every era of the legendary NOW compilation series.
                </flux:text>
            </div>
        </header>
        ...
        <flux:card class="shadow-2xl border-none bg-white/80 dark:bg-zinc-900/80 backdrop-blur-xl ring-1 ring-zinc-200 dark:ring-zinc-800">
            <div class="space-y-5 md:space-y-6">
                <div class="flex flex-col gap-4">
                    {{-- Date picker with Surprise button --}}
                    <div class="w-full">
                        <flux:field>
                            ...
                            <div class="flex gap-2">
                                <div class="flex-1">
                                    <flux:date-picker
                                        ...
                                        with-today
                                        clearable
                                        class="!bg-transparent border-zinc-200 dark:border-zinc-800 focus:ring-blue-500 w-full"
                    ...
                    <flux:button variant="primary" wire:click="search" icon="magnifying-glass" class="w-full py-3 bg-blue-600 hover:bg-blue-700 shadow-lg shadow-blue-500/20 transition-all duration-300 text-base font-semibold">
                        Search
                    </flux:button>
                </div>
    ...
    <div wire:loading.remove wire:target="search">
        ...
        @if($albums && $albums->count() > 0)
            <div class="space-y-10 md:space-y-16">
                @foreach($albums as $album)
                    <div class="space-y-4 md:space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-1000 fill-mode-both" style="animation-delay: {{ $loop->index * 150 }}ms" wire:key="album-{{ $album->id }}">
                        <div class="relative group">
                            <!-- Background Blur Effect -->
                            <div class="absolute -inset-1 bg-gradient-to-r from-blue-600 to-indigo-600 rounded-2xl md:rounded-3xl blur opacity-25 group-hover:opacity-40 transition duration-1000 group-hover:duration-200"></div>
                            
                            <flux:card class="relative p-0 overflow-hidden border-none shadow-2xl bg-white dark:bg-zinc-900 rounded-xl md:rounded-2xl">
                                <div class="grid md:grid-cols-[auto_1fr] items-stretch">
                                    {{-- Album Art - Smaller on mobile --}}
                                    <div class="relative bg-zinc-900 aspect-[4/3] md:aspect-square md:w-80 w-full overflow-hidden">
                                        @php
                                        ...
                                        <div class="h-full w-full flex items-center justify-center" @if(!$artworkResolved) wire:intersect.once="resolveAlbumArtwork({{ $album->id }})" @endif>
                                            ...
                                            @else
                                                <div class="flex flex-col items-center gap-3 md:gap-4 text-zinc-600">
                                                    <flux:icon icon="musical-note" class="size-12 md:size-20" />
                                                    <span class="text-[10px] md:text-xs font-bold uppercase tracking-widest">No Cover Art</span>
                                                </div>
                                            @endif
                                        </div>
                                    ...
                                    {{-- Album Info --}}
                                    <div class="p-5 md:p-10 flex flex-col justify-center space-y-3 md:space-y-6 bg-gradient-to-br from-white to-zinc-50 dark:from-zinc-900 dark:to-zinc-950">
                                        <div class="space-y-1 md:space-y-2">
                                            <flux:text size="sm" class="text-blue-600 dark:text-blue-400 font-bold uppercase tracking-[0.15em] md:tracking-[0.2em] text-[10px] md:text-sm">NOW Collection</flux:text>
                                            <flux:heading size="xl" class="text-2xl sm:text-3xl md:text-5xl font-black tracking-tighter">{{ $album->display_name }}</flux:heading>
                        ...
                        <div class="px-2">
                            @if($album->songs->count() > 0)
                                ...
                                <div class="hidden md:block bg-white/50 dark:bg-white/[0.02] rounded-3xl border border-zinc-200 dark:border-white/5 overflow-hidden backdrop-blur-sm">
        ...
        @else
            ...
            <div class="space-y-8 md:space-y-12 animate-in fade-in slide-in-from-bottom-8 duration-1000 fill-mode-both">
                ...
                <!-- Stats Section -->
                <div class="relative overflow-hidden rounded-2xl md:rounded-[3rem] bg-zinc-900 text-white p-6 md:p-12 shadow-2xl">
                    <div class="absolute top-0 right-0 -translate-y-1/2 translate-x-1/2 size-48 md:size-96 bg-blue-600/20 blur-[60px] md:blur-[100px] rounded-full"></div>
                    <div class="absolute bottom-0 left-0 translate-y-1/2 -translate-x-1/2 size-48 md:size-96 bg-purple-600/20 blur-[60px] md:blur-[100px] rounded-full"></div>
                    
                    @php $stats = $this->stats; @endphp
                    <dl class="relative z-10 grid grid-cols-2 md:grid-cols-4 gap-4 md:gap-8 text-center">
                        <div class="space-y-0.5 md:space-y-1">
                            <dt class="text-zinc-400 uppercase tracking-widest text-[9px] md:text-[10px] font-black">Albums</dt>
                            <dd class="text-2xl md:text-4xl font-black tabular-nums">{{ number_format($stats['total_albums'] ?? 0) }}</dd>
                        </div>
                        <div class="space-y-0.5 md:space-y-1">
                            <dt class="text-zinc-400 uppercase tracking-widest text-[9px] md:text-[10px] font-black">Songs</dt>
                            <dd class="text-2xl md:text-4xl font-black tabular-nums">{{ number_format($stats['total_songs'] ?? 0) }}</dd>
                        </div>
                        <div class="space-y-0.5 md:space-y-1">
                            <dt class="text-zinc-400 uppercase tracking-widest text-[9px] md:text-[10px] font-black">Since</dt>
                            <dd class="text-2xl md:text-4xl font-black tabular-nums">{{ ($stats['date_range']['first'] ?? null)?->year ?? '—' }}</dd>
                        </div>
                        <div class="space-y-0.5 md:space-y-1">
                            <dt class="text-zinc-400 uppercase tracking-widest text-[9px] md:text-[10px] font-black">Latest</dt>
                            <dd class="text-2xl md:text-4xl font-black tabular-nums">{{ ($stats['date_range']['latest'] ?? null)?->year ?? '—' }}</dd>
                        </div>
                    </dl>
                </div>
            </div>
